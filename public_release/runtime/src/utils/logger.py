...
class ColorStreamHandler(logging.StreamHandler):
    mapping = {
        ...
        # "ALERT": f"{Color.RED}[  ALERT ]{Color.RESET}",
        # "ERROR": f"{Color.RED}[  ERROR ]{Color.RESET}",
        # "WARNING": f"{Color.BG_RED}[ WARNING]{Color.RESET}",
    }
...
def get_root_logger(logger_name='dcvic', log_level: Union[int, str]=logging.INFO, log_file=None):
    """Get the root logger.
    The logger will be initialized if it has not been initialized. By default a
    StreamHandler will be added. If `log_file` is specified, a FileHandler will
    also be added.
    Args:
        logger_name (str): root logger name. Default: 'dcvic'.
