...
def read_batch_from_image_path(path: str) -> torch.Tensor:
    """
    get torch batch from image path

    Args:
        path (str) : path of image file
    
    Return:
        img (torch.Tensor) : single batch [1, 3, H, W] RGB, [0, 1] float
    """
    img = cv2.imread(path) # img : numpy, BGR, [0, 255], uint8
    img = img[..., ::-1] # BGR -> RGB
    img = img.astype(np.float32) / 255 # [0,255] uint8 -> [0,1] float
    img = npimg2torch(img) # numpy -> torch [CHW]
