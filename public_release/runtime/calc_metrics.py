...
def get_dataloader(real_path_list: List[str], fake_path_list: List[str], scale: str):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=1, drop_last=False, shuffle=False, num_workers=0
    )
    return dataloader
...
class FIDMetric(BaseMetric):
    ...
    def calc_metric(
        ...
        if len(real_path_list) < 50:
            self.logger.warning(
                f"num_img (={num_img}) is smaller than the usual FID recommendation, "
                "but continuing because this implementation computes FID on extracted patches."
            )
...
def main():
    ...
    metrics_dict = {}
    metric_builders = [
        ("PSNR", lambda: PSNRMetric()),
        ("FID", lambda: FIDMetric(opt.device)),
        ("LPIPS", lambda: LPIPSMetric(opt.device)),
        ("DISTS", lambda: DISTSMetric(opt.device)),
    ]
    failed_metrics = {}
    for metric_name, builder in metric_builders:
        try:
            metrics_dict[metric_name] = builder()
        except Exception as exc:
            failed_metrics[metric_name] = str(exc)
            logger.error(f"Skip {metric_name}: init failed: {exc}")
    ...
    for metrics_name, met_obj in metrics_dict.items():
        ...
        try:
            ...
            failed_metrics[metrics_name] = str(exc)
            logger.error(f"Skip {metrics_name}: run failed: {exc}")
            logger.error(traceback.format_exc())
            continue
        if met_val is not None:
            out_dict[metrics_name] = met_val
    if failed_metrics:
        out_dict["failed_metrics"] = failed_metrics
